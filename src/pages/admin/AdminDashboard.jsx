...
import { adminLogout, getPlayers, getGames, getActiveSeason, addSeason, closeSeason } from '../../data/store.js'
...
export default function AdminDashboard() {
  ...
  function newSeason() {
    ...
    if (name) addSeason(name)
    window.location.reload()
  }

  function endSeason() {
    if (!season) return
    if (confirm(`Закрыть "${season.name}"?`)) { closeSeason(season.id); window.location.reload() }
  }
    ...
    <div className="min-h-screen p-4" style={{background:'#050510'}}>
      <div className="max-w-lg mx-auto">
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          {[
            {val:players.length, label:'Игроков'},
            {val:games.filter(g=>g.status==='completed').length, label:'Игр'},
          ].map(({val,label})=>(
