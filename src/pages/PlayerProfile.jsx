...
import { getPlayers, getActiveSeason, getSeasonGames } from '../data/store.js'
...
export default function PlayerProfile() {
  ...
  const stats = calcPlayerStats(id, games)
  const board = buildLeaderboard(players, games)
      ...
      <div className="card p-5 mb-4 flex flex-col sm:flex-row items-center sm:items-start gap-4 text-center sm:text-left">
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center justify-center sm:justify-start gap-2 mb-3">
            ...
            <span className="badge-season">{season?.name||'—'}</span>
          </div>
          <div className="grid grid-cols-3 gap-3">
            {[
              {val:stats.gamesPlayed, label:'Игр'},
              {val:stats.wins, label:'Побед', color:'#ff00ff'},
              {val:stats.totalKnockouts, label:'Нокауты', color:'#7fff00'},
              {val:stats.top3, label:'Топ-3'},
              {val:stats.best, label:'Рекорд', color:'#00f5ff'},
              {val:stats.seasonPoints, label:'Best 7', color:'#00f5ff'},
            ].map(({val,label,color})=>(
