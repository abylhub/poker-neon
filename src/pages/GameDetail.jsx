      ...
      {top3.length >= 2 && (
        <div className="flex items-end justify-center gap-3 mb-6">
          {[top3[1], top3[0], top3[2]].filter(Boolean).map((r,i)=>{
            ...
            const color = PC[pos]
            return (
              <div key={r.playerId} className={`flex flex-col items-center ${pos===1?'order-2':'pos===2?order-1:order-3'}`} style={{order:pos===1?2:pos===2?1:3}}>
                <Avatar player={p} size={pos===1?56:44} glow={pos===1?'cyan':'cyan'}/>
              ...
              return (
                <tr key={r.playerId} className="border-b border-white/[0.04] last:border-0">
                  <td className="px-3 py-2.5">
                    <span className="font-mono font-bold" style={{color}}>{r.place<=3?['🥇','🥈','🥉'][r.place-1]:r.place}</span>
                  </td>
                  <td className="px-3 py-2.5">
